...
`r if(period !=1) {'The code is generated in Python. Data is extracted with the Devo SDK available on the GitHub account. The user must only fill in the public and secret keys.'}` 


```{r comment=NA, echo=FALSE, eval=(period!=1)}
message('from devo.api import Client',
'\nimport pandas as pd',
...
```{r comment=NA, echo=FALSE, eval=(period!=1)}
message('api = Client(key="fill me",',
              '\nsecret="fill me",',
              '\nurl="https://api-eu.logtrust.com/search/query",',
              '\nuser="user@devo.com",',
              '\napp_name="testing app"')

message('response = api.query(query=',myQ ,',',
                     '\ndates= {\'from\':', params$from, '\'to\':', params$to, '},',
                     '\nresponse="csv",',
                     '\nstream=False')

message('string_list = [x.split(',') for x in response.splitlines()]',
'\nnames = [\'cpuSys\']',
'\ndf = pd.DataFrame(string_list, columns=names)',
'\ndf = df.iloc[1:]',
'\nts = pd.Series(data=df[\'cpuSys\'].tolist(), dtype=np.float)')
```
...
```{r comment=NA, echo=FALSE, eval=(period!=1)}
message('ets_model = ExponentialSmoothing(ts, seasonal_periods = period, seasonal= ty)',
