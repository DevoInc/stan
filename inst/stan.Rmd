...
legend('topleft', legend = c('Time Series', 'Prediction','Confidence Interval','Detected Anomaly'), bty = "n", col = c('black', 'red', 'blue', NA), lty = c(1, 1, NA, NA), pch = c(NA, NA, 19, NA), density=c(0,0,0,NA), fill = c('black', 'red', 'blue', 'grey'), border = c(NA,NA,NA,"grey"), inset = c(-0.05,-0.1), xpd = TRUE, horiz = TRUE)
...
`r if(period !=1) {'This report ends with a simple use case so that the user can get an idea of how to proceed with the knowledge he has acquired with the report. The period length and the three smoothing parameters are used to perform a Holt Winters smoothing automatically, afterwards, the thresholds computed in the report are used to detect anomalies and thus define alerts. The code is available in two languages: R and Python'}` 

`r if(period !=1) {'**TO-DO:** Inyect the table generated back in DEVO!'}` 

`r if(period !=1) {'#### R'}`

```{r, echo=FALSE, eval=(period!=1)}
paste('period <-', period)
paste('alpha <-', hwinters$alpha)
paste('beta <-', hwintersbeta)
paste('gamma <-', hwintersgamma)
paste('type <-', t)
paste('thresholdPercentile <-', threshold)
paste('thresholdConfInt <-', std_res)
```
...
`r if(period !=1) {'#### Python'}`

```{r, echo=FALSE, eval=(period!=1)}
paste('period =', period)
paste('alpha =', hwinters$alpha)
paste('beta =', hwinters$beta)
paste('gamma =', hwinters$gamma)
paste('type =', t)
paste('thresholdPercentile =', threshold)
paste('thresholdConfInt =', std_res)
```
