...
It contains `r length(ts)` observations, going from `r as.POSIXct(params$from / 1000, origin="1970-01-01", tz = "UTC-2")` to `r as.POSIXct(params$to / 1000, origin="1970-01-01", tz = "UTC-2")`. 
...
This technique considers the spectral density of the time series and is based on the fact that the period is the inverse of the frequency. The period length is the frequency that corresponds to the peak value of spectral density.'
```
...
`r if(period !=1 & nullPresence == FALSE & (negativePresence == FALSE)) {'Now that we’ve identified the seasonality type, we can decompose the series.'}`
...
`r if(period !=1 & typ != 'both') {paste('As the seasonality was identified in section 4 as ', type(acfAdd, acfMult),', we apply the corresponding formala. Predictions are made for two periods into the future, in this case we are predicting ', period * 2 , ' points. After fitting the model, it is crucial to calculate goodness-of-fit indicators. The AIC and the BIC are computed in this report. The indicators alone do not provide much information but they are useful for a future model comparison/selection.', sep = '')}`
...
`r if(period !=1) {'Now we use the percentiles technique to identify anomalies based on the smoothed model.
This technique is founded on the heuristic that any values not explained by the model are anomalies. In this report, the 95th percentile is fixed as the threshold for normality, but this percentile will be a variable in future versions. '}`
...
'\nresult_table[\'alertPercentile\'] = np.where(abs(result_table[\'True Series\'] - result_table[\'Smoothed Series\']) > result_table[\'thresholdPercentile\'], \'1\', \'0\')',
'\nresult_table[\'alertConfInt\'] = np.where((result_table[\'True Series\'] < (result_table[\'Smoothed Series\'] - result_table[\'thresholdConfInt\'] * 1.96)), \'1\', \'0\')',
'\nresult_table[\'alertConfInt\'] = np.where((result_table[\'True Series\'] > (result_table[\'Smoothed Series\'] + result_table[\'thresholdConfInt\'] * 1.96)), \'1\', \'0\')',
'\nprint(result_table)')

message('#Inject back into Devo\'s platform',
'\ncon = Sender.from_config({"address": "fill me!", "port": 443, "key": "fill me!", "cert": "fill me!", "chain": "also...fill me!"})',
