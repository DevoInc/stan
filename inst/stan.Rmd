...
`r if(period !=1) {'The code is generated in Python and ready to use. Data is extracted with the Devo SDK publicly available on Devo's GitHub account. The user must only fill in the public and secret API keys.'}` 
...
message('ets_model = ExponentialSmoothing(ts, seasonal_periods = period, seasonal= type)',
